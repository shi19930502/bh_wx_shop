          ...
          <div class="search flex flex-y-center">
            <div class="select-pfs flex-item-4 flex flex-y-center">
              ...
              <div style="margin-left:0.1rem;">
                <yd-badge
                  v-if="!(userInfo.userType=='005')&&!(userInfo.userType=='003')"
                  shape="square"
                  bgcolor="#FE971D"
                  @click.native="toShow1pStores('001')"
                >批发商店铺</yd-badge>
                <yd-badge
                  v-if="userInfo.userType=='003'"
                  shape="square"
                  bgcolor="#FE971D"
                  @click.native="toShow1pStores('003')"
                >配送公司</yd-badge>
              </div>
            </div>
            <div>
            ...
            <div class="menu-tab" style="overflow-x: auto">
              <ul class="flex">
                <template v-for="(d, index) in categoryList">
                  <!-- 需求需要使用vulgo别名字段 name-->
                  <template v-if="d.vulgo">
                    <li
                      :class="d.isActive?'active':''"
                      :style="'min-width: ' + (d.vulgo.length*0.3 +0.5) + 'rem'"
                      @click="menuClick(d)"
                    >
                      <strong>{{d.vulgo}}</strong>
                    </li>
                  </template>
                  <!-- 别名不存在的时候显示名称 -->
                  <template v-else>
                    ...
                    <div class="flex flex-y-center">
                      ...
                      <div class="info flex-item-1">
                        ...
                        <template v-if="d.skuMethodType == 1">
                          <h1>{{d.name+'['+d.skuMeasureName+']'}}</h1>
                        </template>
                        <template v-else>
                          <h1
                            v-if="d.proMeasureName != d.skuMeasureName"
                          >{{d.name+'['+d.skuWeight+d.proMeasureName+'/'+d.skuMeasureName+']'}}</h1>
                          <h1 v-else>{{d.name+'['+d.skuMeasureName+']'}}</h1>
                        </template>
                        ...
                        <!--价格-->
                        <!-- <template v-if="d.price-0 != d.promotionalPricing-0">
                          <span class="active-status">促销</span>
                          <span
                            style="text-decoration: line-through;color: #797D77;"
                          >{{'￥'+d.price}}</span>
                          <h3>{{'￥'+d.promotionalPricing.toFixed(2)}}</h3>
                        </template>
                        <template v-else>
                          <h3>{{'￥'+d.price}}</h3>
                        </template>-->
    ...
    <yd-popup v-model="showShopCar" position="bottom" height="70%">
        ...
        <div v-for="d in alreadyChosenGoods" class="flex flex-x-between flex-y-center">
          <yd-checklist-item :val="d.id" class="flex-item-6">
            <div style="padding: 12px 0;" class="flex flex-x-between flex-y-center">
              <!--商品名&规格名-->
              <template v-if="d.skuMethodType == 1">
                <span>{{d.skuName+'['+d.skuMeasureName+']'}}</span>
              </template>
              <template v-else>
                <span
                  v-if="d.proMeasureName != d.skuMeasureName"
                >{{d.skuName+'['+d.skuWeight+d.proMeasureName+'/'+d.skuMeasureName+']'}}</span>
                <span v-else>{{d.skuName+'['+d.skuMeasureName+']'}}</span>
              </template>
            </div>
            <div></div>
          </yd-checklist-item>
      ...
      <div class="yd-tabbar" slot="bottom">
        <div class="tabbar flex">
          ...
          <div>
            <div v-if="orderTotal<startAmount" class="tabbar-btn flex flex-y-center flex-x-center">
              <div>
                <p>起配金额</p>
                <p>￥{{startAmount}}</p>
              </div>
      ...
      <div class="prodet-info flex flex-y-end">
        ...
        <div class="prodetpn">
          ...
          <template v-if="proDetailParams.skuMethodType == 1">
            <p
              class="prodet-name"
            >{{proDetailParams.name+'['+proDetailParams.skuMeasureName+']'}}</p>
          </template>
          <template v-else>
            <p
              class="prodet-name"
              v-if="proDetailParams.proMeasureName != proDetailParams.skuMeasureName"
            >{{proDetailParams.name+'['+proDetailParams.skuWeight+proDetailParams.proMeasureName+'/'+proDetailParams.skuMeasureName+']'}}</p>
            <p
              class="prodet-name"
              v-else
            >{{proDetailParams.name+'['+proDetailParams.skuMeasureName+']'}}</p>
...
export default {
  ...
  methods: {
    ...
    toShow1pStores(type) {
      ...
      // this.$router.push({ name: "NorthDoorStore" });
    },
    showProDetailfuc(d) {
