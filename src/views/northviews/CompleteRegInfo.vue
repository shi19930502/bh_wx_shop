  ...
  <yd-layout>
    <div id="UserReg">
      ...
      <yd-cell-group>
        <yd-cell-item>
          ...
          <span slot="left">
            <div class="sfbtns">
              <yd-radio-group v-model="form.customerProperty" size="16" dis>
                <yd-radio val="001" disabled>
                  <span class="text">批发商</span>
                </yd-radio>
                <yd-radio val="007" disabled>
                  <span class="text">厂商</span>
                </yd-radio>
                <yd-radio val="002" disabled>
                  <span class="text">二级批发/早市/农贸市场</span>
                </yd-radio>
                <yd-radio val="004" disabled>
                  <span class="text">小区门店</span>
                </yd-radio>
                <yd-radio val="008" disabled>
                  <span class="text">配送公司</span>
                </yd-radio>
                <yd-radio val="003" disabled>
                  <span class="text">餐饮店/团体单位</span>
                </yd-radio>
                <yd-radio val="005" disabled>
            ...
            <div class="sfbtns" style="padding-left:0.15rem">
              <template v-if="!(form.customerProperty=='001'||form.customerProperty=='002')">
                <yd-radio-group v-model="defaultProperty" size="16">
                  <yd-radio :val="d.key" v-for="d in customerTypeList" :key="d.key" disabled>
      ...
      <yd-cell-group>
        <yd-cell-item>
          <span slot="left">
            <div class="lable">
              <!-- <strong v-if="form.customerProperty != '005' && form.property1!='2'">公司名称</strong> -->
              <strong>个人/公司名称</strong>
            </div>
          </span>
          <yd-input v-model="form.name" slot="right" max="20" placeholder="请输入个人/公司名称"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
          <span slot="left">
            <div class="lable">
          ...
          <span slot="left">
            <div class="sfbtns">
              <yd-checkbox-group v-model="businessScope" size="16">
                <yd-checkbox :val="d.code" v-for="d in categoryTypeList" :key="d.code" shape="circle">{{d.name}}</yd-checkbox>
              </yd-checkbox-group>
            </div>
          </span>
        </yd-cell-item>
      ...
      <template v-if="form.customerProperty != ''">
        <yd-cell-group title="完善资料">
          <template v-if="form.customerProperty !='005'">
            <yd-cell-item
              v-if="(!(form.customerProperty == '001' && form.isLocal == '0'))&&(form.property!=2&&defaultProperty!=2)"
            >
              <span slot="left">
                <div class="lable">
            ...
            <yd-cell-item v-if="!(form.customerProperty == '001' && form.isLocal == '0')">
              <span slot="left">
                <div class="lable">
                  <strong>{{form.property!=2&&defaultProperty!=2?'营业执照':'市场证明'}}</strong>
                </div>
              </span>
              <el-upload
                slot="left"
              ...
              >
                <yd-button
                  ...
                  bgcolor="#6FB138"
                  color="#fff"
                >{{form.property!=2&&defaultProperty!=2?'上传营业执照':'上传市场证明'}}</yd-button>
              </el-upload>
            </yd-cell-item>
          ...
          <!-- 清真食品准营证 -->
          <template v-if="form.customerProperty !='005' && form.customerProperty !='003'">
            <yd-cell-item>
              <span slot="left">
                <div class="lable">
    ...
    <div class="yd-tabbar" slot="bottom">
      <div class="flex flex-x-center" style="width: 100%;">
        <div @click="updateCusMerPic" class="loginbtn flex flex-y-center flex-x-center">
          <span>保存信息</span>
        </div>
      </div>
    </div>
...
export default {
  ...
  async created() {
    ...
    if (this.$route.query.typeCode) {
     await this.postRequest("customer/list", {
        ...
        if (resp.data && resp.data.state == 0) {
          if (resp.data.aaData.length>0) {
            ...
            if(customerEntity.businessScope){
              ...
              this.businessScope = customerEntity.businessScope.split("/");
            }
          }
        }
      });
  ...
  methods: {
    ...
    checkForm(isUpdate) {
      ...
      if (this.form.customerProperty != "005" && this.form.property == "") {
        if (
          this.form.customerProperty == "001" ||
          this.form.customerProperty == "002"
        ) {
      ...
      // 一级批发商选择供应商来源
      if (this.form.customerProperty == "001" && this.form.isLocal == "") {
        this.$dialog.alert({
          mes: "请选择供应商品来源"
        });
        return false;
      ...
      if(this.form.customerProperty != '005'){
        ...
        if(this.form.businessLicensePic === ''){
          if(!(this.form.customerProperty == '001' && this.form.isLocal == '0')){
            if(this.form.property!=2&&this.defaultProperty!=2){
              this.$dialog.alert({
                mes: "请上传营业执照"
              });
              return false;
